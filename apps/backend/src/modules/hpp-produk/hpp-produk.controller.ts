...
export const getAllHppProduk = async (
  ...
  try {
    ...
    let idBrand = req.query.id_brand
      ? parseInt(req.query.id_brand as string, 10)
      : undefined;

    if (req.user?.role === "user_brand") {
      idBrand = req.user.id_brand ?? undefined;
    }

    const result = await hppProdukService.getAllHppProduk(
      page,
      limit,
      search,
      idBrand,
...
export const getHppProdukById = async (
  ...
  try {
    const data = await hppProdukService.getHppProdukById(
      parseInt(req.params.id, 10),
    );
    if (
      req.user?.role === "user_brand" &&
      data.id_brand !== req.user.id_brand
    ) {
      throw new AppError(
        "Forbidden: You can only view your assigned brand's data",
        403,
      );
    }
...
export const bulkCreateHppProduk = async (
  req: Request<
    unknown,
    ApiResponse<{ insertedCount: number }>,
    CreateHppProdukRequest[]
  >,
  ...
  try {
    ...
    sendCreated(
      res,
      { insertedCount },
      `Berhasil menambahkan ${insertedCount} produk`,
    );
  } catch (error) {
    next(error);
  }
...
export const updateHppProduk = async (
  ...
  try {
    const data = await hppProdukService.updateHppProduk(
      parseInt(req.params.id, 10),
      req.body,
    );
...
export const clearHppProdukByBrand = async (
  ...
  try {
    ...
    sendSuccess(
      res,
      { deletedCount },
      `Berhasil menghapus ${deletedCount} HPP Produk untuk brand ini`,
    );
  } catch (error) {
    next(error);
  }
