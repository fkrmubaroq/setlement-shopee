    ...
    <Card className="border-none shadow-md">
      <CardHeader className="flex flex-row items-center justify-between pb-2 border-b">
        <div className="space-y-1">
          ...
          <p className="text-sm text-gray-500">Kelola semua brand anda disini.</p>
        </div>
        <BrandFormSheet />
      </CardHeader>
          ...
          <Table>
            <TableHeader className="bg-gray-50 dark:bg-gray-900">
              <TableRow>
                ...
                <TableHead className="hidden md:table-cell">Mulai Dibuat</TableHead>
            ...
            <TableBody>
              {isLoading ? (
                ...
                brands.map((brand) => (
                  <TableRow key={brand.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                    <TableCell className="text-center font-medium">{brand.id}</TableCell>
                    <TableCell className="font-semibold text-gray-700 dark:text-gray-200">
                      {brand.nama_brand}
                    </TableCell>
                    <TableCell className="text-gray-500 hidden md:table-cell">
                      {brand.created_at ? new Date(brand.created_at).toLocaleDateString("id-ID") : "-"}
                    </TableCell>
                    <TableCell className="text-right">
